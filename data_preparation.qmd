---
title: "Data Preparation: Collaborative Papers from GEN26 Universities"
format:
  html:
    toc: true
    toc-depth: 3
...
### Get Works, Authors and Affilitations
...
```{r}
# Create address column combining city and country
c_preview <- c_preview %>%
  mutate(address = paste(affil, city, country, sep = ", "))

# Geocode to get latitude and longitude
c_preview_geocoded <- c_preview %>%
...
```{r}
write.csv(w_preview, file="w_preview.csv", row.names = FALSE)
write.csv(a_preview, file="a_preview.csv", row.names = FALSE)
write.csv(f_preview, file="f_preview.csv", row.names = FALSE)
write.csv(c_preview, file="c_preview.csv", row.names = FALSE)
write.csv(c_preview_geocoded, file="c_preview_geocoded.csv", row.names = FALSE)
```
